...
class ArmX_Image
{
    ...
    protected function __construct(SplFileInfo $file)
    {
        ...
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new ArmX_Exception(_t('非法的图片文件'));
        }
        ...
        if ('gif' == $this->info['type']) {
            $this->gif = new ArmX_Image_Gif($file->getPathname());
        ...
        if (empty($this->im)) {
            throw new ArmX_Exception(_t('创建图像资源失败!'));
        }
    ...
    public static function open($file)
    {
        ...
        if (!$file->isFile()) {
            throw new ArmX_Exception(_t('图像文件不存在'));
        }
        ...
        do {
            ...
            switch ($direction) {
                ...
                case self::FLIP_Y:
                    ...
                    break;
                default:
                    throw new ArmX_Exception(_t('不支持的翻转类型'));
            }
    ...
    public function thumb($width, $height, $type = self::THUMB_SCALING)
    {
        ...
        switch ($type) {
            ...
            case self::THUMB_FIXED:
                ...
                break;
            default:
                throw new ArmX_Exception(_t('不支持的缩略图裁剪类型'));
        }
        /* 裁剪图像 */
    ...
    {
        if (!is_file($source)) {
            throw new ArmX_Exception(_t('水印图像不存在'));
        }
        //获取水印图像信息
        $info = getimagesize($source);
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new ArmX_Exception(_t('非法水印文件'));
        }
        //创建水印图像资源
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new ArmX_Exception(_t('不支持的水印位置类型'));
                }
        }
        do {
            //添加水印
     ...
     *
     * @return $this
     * @throws ArmX_Exception
     */
    public function text($text, $font, $size, $color = '#00000000',
        $locate = self::WATER_SOUTHEAST, $offset = 0, $angle = 0) {

        if (!is_file($font)) {
            throw new ArmX_Exception(_t("不存在的字体文件：{$font}"));
        }
        //获取文字信息
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new ArmX_Exception(_t('不支持的文字位置类型'));
                }
        }
        /* 设置偏移量 */
        if (is_array($offset)) {
        ...
        if (is_string($color) && 0 === strpos($color, '#')) {
            ...
            throw new ArmX_Exception(_t('错误的颜色值'));
        }
        do {
            /* 写入文字 */
